...
const getFiles = (fileTitle:string, page:number, size:number): Promise<any> => {
    return http.get(`/updateFileDb/fileDb?fileTitle=${fileTitle}&page=${page}&size=${size}`);
  };
  
  // 상세 조회
  const getFileDb = (uuid:any): Promise<any> => {
    return http.get(`/updateFileDb/fileDb/get/${uuid}`);
  };
  ...
  const upload = (updateFileDb:IFileDb,fileDb:any): Promise<any> => {
    ...
    return http.post("/updateFileDb/fileDb/upload", formData, {
      headers:{
  ...
  const updateFileDb = (uploadFileDb:IFileDb, fileDb:any): Promise<any> => {
    ...
    return http.put(`/updateFileDb/fileDb/${uploadFileDb.uuid}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data"
      },
    });
  ...
  const deleteFile = (uuid:any):Promise<any> => {
    return http.delete<any>(`/updateFileDb/fileDb/deletion/${uuid}`);
  };
